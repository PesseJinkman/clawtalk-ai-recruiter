  ...
  return (
    <div className="page-container">
      <div className="content-wrapper max-w-3xl">
        <Link to={`/jobs/${id}`} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6 transition-colors animate-fade-in-left">
        ...
        {candidate.scores && (
          <div className="card-surface mb-6 animate-fade-in" style={{ animationDelay: '150ms' }}>
            <div className="flex items-baseline gap-2 mb-5">
              <span className="text-4xl font-bold text-foreground tabular-nums">{candidate.scores.overall.toFixed(1)}</span>
        ...
        <div className="card-surface animate-fade-in" style={{ animationDelay: '350ms' }}>
          <h2 className="font-semibold text-foreground mb-4">Call Transcript</h2>
          {!callCompleted || transcript.length === 0 ? (
            ...
            <div className="space-y-3">
              {transcript.map((entry, i) => (
                ...
                >
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${entry.role === 'candidate' ? 'bg-primary text-primary-foreground rounded-br-md' : 'bg-muted text-foreground rounded-bl-md'}`}>
                    <p className={`text-[10px] font-medium mb-1 ${entry.role === 'candidate' ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>
