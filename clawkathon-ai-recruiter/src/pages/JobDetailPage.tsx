...
import { ArrowLeft, Download, Phone, Eye, Upload, Play, Users, PhoneCall, CheckCircle, ArrowUpRight } from 'lucide-react';
      ...
      <div className="content-wrapper">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8 animate-fade-in">
          <div>
            ...
            <p className="text-sm text-muted-foreground mb-2">{job.hr_phone}</p>
        ...
        {job.stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Total Candidates', value: job.stats.total, icon: Users },
              { label: 'Currently Calling', value: job.stats.calling, icon: PhoneCall },
              { label: 'Completed', value: job.stats.completed, icon: CheckCircle },
              { label: 'Advancing', value: job.stats.advance, icon: ArrowUpRight, green: true },
            ].map((s, i) => (
              <div key={s.label} className="card-surface text-center animate-slide-up" style={{ animationDelay: `${100 + i * 80}ms` }}>
                <s.icon className={`h-5 w-5 mx-auto mb-2 ${s.green ? 'text-green-600' : 'text-muted-foreground'}`} />
                <p className={`text-2xl font-bold tabular-nums ${s.green ? 'text-green-600' : 'text-foreground'}`}>{s.value}</p>
        ...
        <div className="card-surface mb-6 animate-fade-in" style={{ animationDelay: '200ms' }}>
          <h2 className="font-semibold text-foreground mb-4 flex items-center gap-2"><Upload className="h-4 w-4" /> Upload Resumes</h2>
          <input ref={fileRef} type="file" multiple accept=".pdf,.docx" onChange={handleFileChange} className="block text-sm text-muted-foreground file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-primary file:text-primary-foreground hover:file:opacity-90 file:cursor-pointer file:transition-opacity cursor-pointer" />
          {files.length > 0 && (
          ...
          {uploadMsg && <p className={`text-sm mt-3 animate-fade-in ${uploadMsg.includes('failed') || uploadMsg.includes('Failed') ? 'text-destructive' : 'text-green-600'}`}>{uploadMsg}</p>}
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-left">
                  ...
                  <th className="pb-3 font-medium text-muted-foreground">Recommendation</th>
              ...
              <tbody>
                {candidates.map((c) => (
                  <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors duration-150">
                    ...
                    <td className="py-3 pr-4"><StatusBadge value={c.status} type="candidate" /></td>
                    <td className="py-3 pr-4 tabular-nums">{c.scores ? `${c.scores.overall.toFixed(1)} / 10` : '—'}</td>
                    <td className="py-3 pr-4">{c.recommendation ? <StatusBadge value={c.recommendation} type="recommendation" /> : '—'}</td>
                    <td className="py-3 text-right space-x-2">
                      ...
                      <Link to={`/jobs/${job.id}/candidates/${c.id}`} className="text-primary hover:underline text-xs inline-flex items-center gap-1 transition-colors">
